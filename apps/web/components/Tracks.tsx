...
import { Track, Problem, Categories, UserProgress } from "@prisma/client";
...
export const Tracks = ({ tracks, categories, userProgress }: TracksWithCategoriesProps) => {
        ...
        {loading ? (
          ...
          visibleTracks.map((t) => (
            <motion.li key={t.id} className="w-full" variants={{ hidden: { opacity: 0 }, show: { opacity: 1 } }}>
              <TrackCard2 track={t} userProgress={userProgress.filter((up) => up.trackId === t.id)} />
            </motion.li>
          ))
        )}
      </motion.ul>
